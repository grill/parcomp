...
void prefixsums(ATYPE** x, int n) {
  //[1] + [2] (+) [3] + [4]
}

//prefis algos from 
cilk void datapara(int i, int j, int unit, int a[], int (*f)(int)) {
  if (j-i<=unit) {
    int k;
    for (k=i; k<j; k++) a[k] = f(k);
  } else {
    spawn datapara(i,j/2,unit,a,f);
    spawn datapara(j/2+1,j,unit,a,f);
    sync;
  }
}
